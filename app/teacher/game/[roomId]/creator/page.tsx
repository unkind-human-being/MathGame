...
export default function CreatorScreen() {
  ...
  // loading state for Review Answers (now used inside leaderboard popup)
  ...
  /* ========== REVIEW ANSWERS (BROADCAST TO PLAYERS) ========== */
  async function reviewAnswers() {
    setOpeningReview(true);
    try {
      // Player page listens to this flag
      await updateDoc(doc(db, "rooms", roomId), {
        reviewOpen: true,
      });
    } catch (err) {
      console.error("Error opening review answers:", err);
      alert("Failed to open review answers. Check console.");
    } finally {
    ...
    <main style={page}>
      ...
      {roomInfo && (
        <div style={roomBox}>
          ...
          <p style={{ opacity: 0.6 }}>
            MODE: <b>{roomInfo.mode || "Waiting"}</b>
          </p>
        </div>
      )}
      ...
      {showLeaderboard && (
        <div style={popupBg}>
          ...
          >
            ...
            {/* New Review Answers button inside leaderboard popup */}
            <button
              style={btn("#9b8bff")}
              disabled={openingReview}
              onClick={reviewAnswers}
            >
              {openingReview ? "Opening Review..." : "Review Answers 🔍"}
            </button>
